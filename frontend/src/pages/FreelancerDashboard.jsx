  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const gigsRes = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/services/my-gigs`,
        { headers }
      );
      ...
      const sellerOrders = Array.isArray(ordersData)
        ? ordersData.filter((order) => order.sellerId?._id === user.id)
        : [];
  ...
  const handleDeleteGig = async (gigId) => {
    ...
    try {
      const res = await fetch(
        `${process.env.REACT_APP_SERVER_URL}/api/services/${gigId}`,
        {
          method: "DELETE",
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={sectionStyle}>
        ...
        {orders.length === 0 ? (
          <p style={{ color: "#666", textAlign: "center", padding: "2rem" }}>
            No orders yet. Your orders will appear here when clients purchase
            your gigs.
          </p>
        ) : (
          <table style={tableStyle}>
