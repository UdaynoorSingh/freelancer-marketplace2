...
const ClientDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const buyerOrders = Array.isArray(ordersData)
        ? ordersData.filter((order) => order.buyerId?._id === user.id)
        : [];
      ...
      const totalSpent = buyerOrders
        ...
        .reduce(
          (sum, order) => sum + (order.serviceId?.price || order.amount || 0),
          0
        );
  ...
  return (
    <div style={containerStyle}>
      <div style={headerStyle}>
        <h1
          style={{ color: "#1dbf73", fontWeight: 700, marginBottom: "0.5rem" }}
        >
      ...
      <div style={sectionStyle}>
        ...
        {purchasedGigs.length === 0 ? (
          <p style={{ color: "#666", textAlign: "center", padding: "2rem" }}>
            You haven't purchased any gigs yet. Start exploring and find the
            ...
            <tbody>
              {purchasedGigs.map((order) => (
                <tr key={order._id}>
                  ...
                  <td style={tdStyle}>
                    ₹{order.serviceId?.price || order.amount || 0}
                  </td>
                  <td style={tdStyle}>
      ...
      <div style={sectionStyle}>
        ...
        {favorites.length === 0 ? (
          <p style={{ color: "#666", textAlign: "center", padding: "2rem" }}>
            You haven't added any gigs to your favorites yet. Start exploring
            and save your favorite services!
          </p>
        ) : (
          <div>
            {favorites.map((gig) => (
              <div key={gig._id} style={gigCard}>
                ...
                >
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      <span style={{ color: "#1dbf73", fontWeight: 600 }}>
                        ₹{gig.price}
                      </span>
