    ...
    <div style={cardStyle}>
      ...
      <div style={{ padding: '0 1.2rem 1.2rem 1.2rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={sellerRow}>
          ...
          {gig.seller?.email && (
            <span style={{ color: textSecondary, fontSize: '0.97rem', marginLeft: 8 }}>
              {gig.seller.email}
            </span>
          )}
        ...
        <div style={{ color: textSecondary, fontSize: '0.98rem', marginBottom: 8 }}>{gig.description}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginTop: 8 }}>
          ...
          <span style={{ color: textPrimary, fontWeight: 600, marginLeft: 4 }}>{avgRating}</span>
          <span style={{ color: textSecondary, fontSize: '0.95rem', marginLeft: 4 }}>({reviewCount})</span>
          <span style={{ marginLeft: 'auto', color: textSecondary, fontSize: '0.95rem', marginRight: 4 }}>From</span>
          <span style={{ color: textPrimary, fontWeight: 700, fontSize: '1.1rem' }}>₹{gig.price}</span>
        </div>
        ...
        {!isOwner && (
          <>
            ...
            {purchaseMsg && <div style={{ color: purchaseMsg.includes('success') ? successColor : 'red', marginTop: 8 }}>{purchaseMsg}</div>}
          </>
        )}
      ...
      <div style={{ marginTop: 32 }}>
        <h3 style={{ color: successColor, fontWeight: 700, marginBottom: 10 }}>Reviews</h3>
        ...
        {reviews.map((r, i) => (
          <div key={i} style={{ background: '#f7f7f7', borderRadius: 8, padding: '1rem', marginBottom: 12 }}>
            ...
            <div style={{ color: textSecondary, fontSize: '1rem' }}>{r.comment}</div>
            <div style={{ color: textSecondary, fontSize: '0.92rem', marginTop: 2 }}>{new Date(r.timestamp).toLocaleString()}</div>
          </div>
        ))}
        {canReview && !showReviewForm && (
          <button style={{ background: successColor, color: '#fff', border: 'none', borderRadius: 8, padding: '0.7rem 1.5rem', fontWeight: 700, fontSize: '1rem', cursor: 'pointer', marginTop: 10 }} onClick={() => setShowReviewForm(true)}>Leave a Review</button>
        )}
        {showReviewForm && (
          <form onSubmit={handleReviewSubmit} style={{ marginTop: 12, background: '#fff', borderRadius: 8, padding: '1rem', boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
            ...
            <button type="submit" style={{ background: successColor, color: '#fff', border: 'none', borderRadius: 8, padding: '0.7rem 1.5rem', fontWeight: 700, fontSize: '1rem', cursor: 'pointer' }}>Submit</button>
            {reviewMsg && <div style={{ color: reviewMsg.includes('submitted') ? successColor : 'red', marginTop: 8 }}>{reviewMsg}</div>}
          </form>
        )}
      </div>
      ...
      {showDeleteModal && (
        <div style={modalOverlay}>
          <div style={modalBox}>
            <h3 style={{ color: dangerColor, marginBottom: 16 }}>Delete Gig?</h3>
        ...
        <div style={modalOverlay}>
          <div style={{ ...modalBox, maxWidth: 600, width: '90%' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              <h3 style={{ color: textPrimary, margin: 0 }}>Complete Payment</h3>
              <button
            ...
            <PaymentForm
              ...
              onSuccess={(order) => {
                ...
                // You can navigate to order details or dashboard here
              }}
              onError={(error) => {
