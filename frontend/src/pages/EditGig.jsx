...
const EditGig = () => {
    ...
    return (
        <form style={formStyle} onSubmit={handleSubmit} encType="multipart/form-data">
            <h2 style={{ color: '#1dbf73', fontWeight: 700, textAlign: 'center' }}>Edit Gig</h2>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: 8 }}>
                {existingImages.length === 0 && <span style={{ color: '#888' }}>No images</span>}
                {existingImages.map(img => (
                    <img key={img} src={`${process.env.REACT_APP_SERVER_URL}/uploads/${img}`} alt="gig" style={{ width: 60, height: 40, objectFit: 'cover', borderRadius: 6 }} />
                ))}
            </div>
            <label style={labelStyle}>Replace Images (up to 5)</label>
            <input style={inputStyle} name="images" type="file" accept="image/*" onChange={handleChange} multiple />
            <button style={btnStyle} type="submit" disabled={loading}>{loading ? 'Updating...' : 'Update Gig'}</button>
        </form>
    );
};
