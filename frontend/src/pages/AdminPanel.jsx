...
const AdminPanel = () => {
    ...
    const deleteUser = async (userId) => {
        ...
        const res = await fetch(`${process.env.REACT_APP_SERVER_URL}/api/admin/users/${userId}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` },
        });
        if (res.ok) {
            ...
            <tbody>
                {users.map(user => (
                    <tr key={user._id}>
                        ...
                        <td style={tdStyle}>
                            <button style={deleteBtnStyle} onClick={() => deleteUser(user._id)}>Delete</button>
                        </td>
                    </tr>
                ))}
            </tbody>
            ...
            <tbody>
                {gigs.map(gig => (
                    <tr key={gig._id}>
                        ...
                        <td style={tdStyle}>
                            <button style={deleteBtnStyle} onClick={() => deleteGig(gig._id)}>Delete</button>
                        </td>
                    </tr>
                ))}
            </tbody>
            ...
            <tbody>
                {reviews.map(review => (
                    <tr key={review._id}>
                        ...
                        <td style={tdStyle}>{new Date(review.timestamp).toLocaleString()}</td>
                        <td style={tdStyle}>
                            <button style={deleteBtnStyle} onClick={() => deleteReview(review._id)}>Delete</button>
                        </td>
                    </tr>
                ))}
            </tbody>
    ...
    return (
        <div style={cardStyle}>
            <h2 style={{ color: '#1dbf73', fontWeight: 700, marginBottom: 24 }}>Admin Panel</h2>
            {msg && <div style={{ color: msg.includes('deleted') || msg.includes('completed') ? '#1dbf73' : 'red', marginBottom: 12 }}>{msg}</div>}
